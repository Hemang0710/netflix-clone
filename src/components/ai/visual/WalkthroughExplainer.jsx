...
export default function WalkthroughExplainer({
  ...
  const { submitFeedback, startTracking, isSubmitting } = useExplanationFeedback()
  ...
  async function handleFeedback(score) {
    await submitFeedback("walkthrough", score, contentId, chapterTitle)
  ...
  return (
    <div className="space-y-3">
          ...
          <div className="flex gap-1">
            {[1, 2, 3, 4, 5].map((score) => (
              <button
                ...
                style={{
                  backgroundColor: score <= 2 ? "#7f1d1d" : score <= 3 ? "#9a3412" : "#dc2626",
                  color: "#fca5a5",
                  opacity: 0.6,
                }}
              >
      ...
      {feedbackGiven && (
        <div className="pt-2 text-center text-xs text-green-400">
