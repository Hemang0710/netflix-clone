...
import DebateMode from "@/components/ai/DebateMode"
import AIChatSidebar from "@/components/ai/AIChatSidebar"
import NotesPanel from "@/components/notes/NotesPanel"
import ConceptMapPanel from "@/components/ai/ConceptMapPanel"
import CommentsSection from "@/components/social/CommentsSection"
import DifficultyBadge from "@/components/ui/DifficultyBadge"
import TranscriptExplainer from "@/components/video/TranscriptExplainer"
import StudyGroupPanel from "@/components/social/StudyGroupPanel"
import ComprehensionCheckpoint from "@/components/learning/ComprehensionCheckpoint"
import ActiveLearnersBar from "@/components/social/ActiveLearnersBar"
import BuddySessionIndicator from "@/components/social/BuddySessionIndicator"
import GroupStudyRoomPanel from "@/components/social/GroupStudyRoomPanel"
...
export default function WatchPageClient({ content, chapters, creatorName, userId }) {
  const videoRef = useRef(null)
  const [activeTab, setActiveTab] = useState("overview")

  useVideoProgress(videoRef, content.id)
  const { confusionSignal, clearSignal } = useVideoConfusion(videoRef, chapters)
  const { checkpointTrigger, markPassed, clearCheckpoint } = useChapterCheckpoints(
    content.id,
    chapters,
    videoRef,
    userId
    ...
    <div className="max-w-6xl mx-auto px-4 md:px-6 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div>
            ...
            <div className="mb-3">
              <ActiveLearnersBar contentId={content.id} />
            </div>
          ...
          {activeTab === "room" && (
            <GroupStudyRoomPanel contentId={content.id} videoRef={videoRef} userId={userId} />
          )}
        </div>

        {/* Right sidebar */}
        <div className="space-y-4">
          {/* Buddy indicator would go here if sessionId exists */}
      ...
      {checkpointTrigger && (
        <ComprehensionCheckpoint
          contentId={content.id}
          chapterIndex={checkpointTrigger.chapterIndex}
          chapterTitle={checkpointTrigger.chapterTitle}
          chapters={chapters}
          isFirstWatch={checkpointTrigger.isFirstWatch}
          onPass={(idx, score) => markPassed(idx, score)}
          onSkip={clearCheckpoint}
          transcript={content.transcript}
        />
      )}
    </div>
