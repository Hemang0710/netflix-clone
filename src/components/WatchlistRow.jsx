    ...
    return (
        <div className="relative shrink-0 w-36 h-52 rounded-md overflow-hidden cursor-pointer group">
            ...
            <button 
                ...
                className="self-end bg-white text-black text-xs font-bold px-2 py-1 rouded hover:bg-zinc-200 transition-colors">
                ✕ Remove
                </button>
...
export default function WatchlistRow({title, movies}) {
    ...
    // Show nothing if watchlist is empty
    if(!movies?.length && ! watchlist.length) return null

    return (
        <div className="mb-8 px-12">
            <h2 className="text-white text-xl font-bold mb-4">{title}</h2>
            <div className="flex gap-3 overflow-auto pb-4 scrollbar-hide">
                {movies.map(movie => (
